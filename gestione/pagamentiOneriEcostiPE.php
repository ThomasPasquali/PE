...
<html>
	...
	<?php 
		...
		$pratica = $c->getDatiPraticaPE($pratica);
	?>
		...
		<div id="main">
			...
			<div id="calcoli">
				<?php 
				$calcoli = $c->db->ql('SELECT * FROM pe_ou_cc WHERE Pratica = ? ORDER BY Numero_revisione', [$pratica['ID']]);
				if(count($calcoli) == 0)
				...
				foreach ($calcoli as $calcolo) {
					...
					<div class="calcolo<?= $calcolo['Attivo']=='N'?' inattivo':'' ?>">
						<button type="button" class="btnAttivaDisattiva" data-attivo="<?= $calcolo['Attivo']=='S'?'true':'false' ?>" data-idcalcolo="<?= $calcolo['ID'] ?>" data-peOtec="pe"><?= $calcolo['Attivo']=='N'?'Attiva':'Disattiva' ?> pagamenti</button>
						<button type="button" class="btnElimina" data-idcalcolo="<?= $calcolo['ID'] ?>" data-peOtec="pe" style="background-color:red; font-size:0.8rem;">Elimina calcolo</button>
						<div class="calcolo-sub">
							...
							<div id="pagamenti">
								...
								<?php 
								$pagamentiOU = $c->db->ql('SELECT * FROM pe_pagamenti_ou WHERE Ou_cc = ? ORDER BY Data', [$calcolo['ID']]);
								echo "<script>";
								foreach ($pagamentiOU as $pagamento) {
									$data = explode('-', $pagamento['Data']);
									echo "addFieldsPagamentoOU($pagamento[Importo], '".(count($data)==3?'versati il '.$data[2].'/'.$data[1].'/'.$data[0]:'')."', $pagamento[ID], $calcolo[ID], 'pe');";
								}
								echo "</script>";
								?>
								<div>
									...
									<button type="button" onclick="aggiungiPagamentoOU(<?= $calcolo['ID'] ?>, 'pe');">Aggiungi pagamento</button>
								</div>
								<h4>Costo di costruzione</h4>
								<div id="pagamentiCC<?= $calcolo['ID'] ?>" class="pagamenti-sub"></div>
								<?php 
								$pagamentiCC = $c->db->ql('SELECT * FROM pe_pagamenti_cc WHERE Ou_cc = ? ORDER BY Data', [$calcolo['ID']]);
								echo "<script>";
								foreach ($pagamentiCC as $pagamento) {
									$data = explode('-', $pagamento['Data']);
									echo "addFieldsPagamentoCC($pagamento[Importo], '".(count($data)==3?'versati il '.$data[2].'/'.$data[1].'/'.$data[0]:'')."', $pagamento[ID], $calcolo[ID], 'pe');";
								}
								echo "</script>";
								?>
								<div>
									...
									<button type="button" onclick="aggiungiPagamentoCC(<?= $calcolo['ID'] ?>, 'pe');">Aggiungi pagamento</button>
								</div>
							</div>
						</div>
					</div>
